...
import "./Quality.css";
import qaImg1 from "../Assests/QA_Testing.jpeg";
import qaImg2 from "../Assests/QA_Testing_Services.jpeg";
...
const Quality = () => {
  return (
    <div className="quality-container">
      {/* Header Section */}
      <div className="quality-section quality-intro">
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <h2 className="quality-title">Quality Analyst</h2>
          <p className="quality-description">
            Every organization needs a proper software testing process to keep
            track of product quality. Onenessweb Solution believes in
            ‘Prevention Over Inspection’. We handle QA Testing services as a
            vital part of every project. Our Quality Analyst professionals
            deliver solutions with minimum risks. We undertake parameters like
            scalability, behavioral, functionality, compatibility, load, and
            performance testing as a part of our development process. You can
            also hire dedicated best QA Experts.
          </p>
        </motion.div>
      </div>
      ...
      {servicesData.map((service, index) => (
        <div
          ...
          className={`quality-section ${
            index % 2 !== 0 ? "quality-section-reverse" : ""
          }`}
        >
          <motion.div
            className="quality-text"
            ...
            viewport={{ once: true }}
          >
            <h2 className="quality-title">{service.title}</h2>
            {service.text.map((para, i) => (
              <p key={i} className="quality-description">
                {para}
              </p>
            ))}
          </motion.div>
